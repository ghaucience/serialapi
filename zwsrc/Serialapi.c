...
/*
#ifdef __ASIX_C51__
#define LIBSA_PRINTF LIBSA_PRINTF
#else
#define LIBSA_PRINTF(f, ...) LIBSA_PRINTF("\033[36;1m SerialAPI: " f "\033[0m",  ##__VA_ARGS__ )
#endif
*/

#ifdef LIBDEBUG
#define LIBSA_PRINTF printf
#else
#define LIBSA_PRINTF(...) 
#endif
...
#else
#define ASSERT(a) {if(!(a)) LIBSA_PRINTF("Assertion failed at %s:%i\n", __FILE__,__LINE__ );}
#endif
...
static void dump_serbuf()  {
  ...
  LIBSA_PRINTF("------------Serial Buffer --------------\n");
  for(i=0; i < serBufLen; i++) {
    LIBSA_PRINTF("%02x ", serBuf[i]);
  }
  LIBSA_PRINTF("\n");
  LIBSA_PRINTF("------------------------------------\n");
}*/
...
{
  if(capabilities.supported_serialapi_bitmask & subcmd) {
    return 1;
  } else {
    return 0;
  }
...
BOOL SerialAPI_Init(const char* serial_port, const struct SerialAPI_Callbacks* _callbacks )
{
  ...
  int i;
    ...
    if(!SendFrameWithResponse(FUNC_ID_SERIAL_API_GET_CAPABILITIES, 0, 0 , buffer, &byLen )) {
      LIBSA_PRINTF("SendFrameWithResponse(FUNC_ID_SERIAL_API_GET_CAPABILITIES,...) failed in SerialAPI_Init()\n");
      ASSERT(0);
      return FALSE;
    }
    memcpy(&capabilities, &(buffer[IDX_DATA]), byLen -IDX_DATA);
    //LIBSA_PRINTF("Serial API version     : %d.%.2d\n",capabilities.appl_version,capabilities.appl_revision);
    ...
    if(SupportsCommand(FUNC_ID_SERIALAPI_SETUP)) {
      if(!SendFrameWithResponse(FUNC_ID_SERIALAPI_SETUP, buffer, idx, buffer, &byLen )) {
        LIBSA_PRINTF("SendFrameWithResponse(FUNC_ID_SERIALAPI_SETUP,...) failed in SerialAPI_Init()\n");
        ASSERT(0);
        return FALSE;
      }
      memcpy(&capabilities.supported_serialapi_bitmask, &(buffer[IDX_DATA + 1]), byLen - IDX_DATA - 1);
    }
    ...
    if(type < sizeof(zw_lib_names) /sizeof(char*)) {
      LIBSA_PRINTF("Z-Wave library version : %s\n",&buf[7]);
      LIBSA_PRINTF("Z-Wave library type    : %s\n",zw_lib_names[type]);
    }
    //if( type != ZW_LIB_CONTROLLER_BRIDGE && type != ZW_LIB_CONTROLLER_STATIC) {
    if( type != ZW_LIB_CONTROLLER_BRIDGE) {
      LIBSA_PRINTF("Z-Wave dongle MUST be a Bridge library\n");
      return FALSE;
    }
...
static void QueueFrame() {
  ...
  if(rxQueue_Len() >= MAX_RXQUEUE_LEN)
  {
	  ...
	  return;
		...
		if(l == NULL)
		{
			LIBSA_PRINTF("QueueFrame:malloc failed.Packet Loss !!!\r\n");
			return;
		}
		l->data = zmalloc(serBufLen);
		if(l->data == NULL)
		{
		LIBSA_PRINTF("QueueFrame: malloc failed-2.Packet Loss !!!\r\n");
		return;
		}
  #else
    ...
    if(ret != conIdle) {
      break;
		...
		if(PORT_TIMER_EXPIRED(t)) {
		  LIBSA_PRINTF("WaitResponse Timeout\n");
		  ret=conRxTimeout;
		  break;
		}
	}
  ...
  BYTE  param_len)
{
	...
	if(!SupportsCommand(cmd)) {
	  LIBSA_PRINTF("Command: 0x%x is not supported by this SerialAPI\n",(unsigned)cmd);
	  ASSERT(0);
	  return conTxErr;
	}
  ...
  ConTxFrame(cmd, REQUEST, param_buf, param_len);
	for(i=0; i < 20; i++) {
      ...
      LIBSA_PRINTF("Got RESPONSE frame while sending.... \n");
//      dump_serbuf();
      ...
      break;
		...
		//LIBSA_PRINTF("Retransmission %i of %2x %s\n",i,cmd, ConTypeToStr(ret));

		LIBSA_PRINTF("Retransmission %d of 0x%02x\n",i,cmd);

#ifdef __ASIX_C51__
		  //When SSL handshake is in progress, don't retry sending out.
		  if((gIsSslHshakeInProgress == 1) && (i >= 3))
		  {
			  LIBSA_PRINTF("SSL in progress. No serial retries!!!!\r\n" );
			  break;
		  }
#endif
		...
		if((i & 7) == 7) {
		  LIBSA_PRINTF("Reopening serial port\n");
		  SerialClose();
			extern void SerialReOpen();
			SerialReOpen();
	...
	LIBSA_PRINTF("Unable to send frame!!!!!!\n");
	//ASSERT(0);
	return ret;
}
      ...
      if(ret==conFrameReceived) {
        if(serBuf[1] == REQUEST) {
          ...
          if( serBuf[ IDX_CMD ] == cmd ) {
            ...
            //Dispatch(serBuf);
            LIBSA_PRINTF("Got new frame while sending %d\n",i);
          }
        }
      } else {
        LIBSA_PRINTF("Unexpected receive state! %s\n",ConTypeToStr(ret));
      }
    }
  }
  LIBSA_PRINTF("SendFrameWithResponse() returning failure\n");
  return 0;
}
...
DetectBufferOverflow(uint16_t len)        /* IN  length of frame */
{
    if (len > sizeof(pCmd))
    {
      LIBSA_PRINTF("Buffer overflow, dropping SerialAPI frame\n");
      return TRUE;
    }
    return FALSE;
}
...
static void Dispatch( BYTE *pData , uint16_t len)
{
  ...
  LIBSA_PRINTF("[DISP]:%02X\n",pData[ IDX_CMD ]);
	{
		int ii = 0;
		LIBSA_PRINTF("[DATA]:");
		for (ii = 0; ii < len; ii++) {
			LIBSA_PRINTF("[%02X] ", pData[ii]&0xff);
			if ( (ii + 1) % 20 == 0) {
				LIBSA_PRINTF("\n[DATA]:");
			}
		}
  ...
  switch ( pData[ IDX_CMD ] )
  {
    ...
    case FUNC_ID_ZW_APPLICATION_CONTROLLER_UPDATE:
      {
          ...
          if (bStatus == UPDATE_STATE_NODE_INFO_FOREIGN_HOMEID_RECEIVED)
          {
            ...
            uint8_t idx = IDX_DATA + 1;
						...
						pAppNodeInfo = pAppNodeInfo;
						nodeInfoLen = nodeInfoLen;
    ...
    case FUNC_ID_ZW_SEND_DATA_BRIDGE:
      {
        //LIBSA_PRINTF("SendDataCallBack\n");
          if ( cbFuncZWSendDataBridge != NULL ) {
            static TX_STATUS_TYPE txStatusReport;
            assert(len == 24);
            txStatusReport.bRepeaters = pData[IDX_DATA+4];
            txStatusReport.transmitTicks = pData[IDX_DATA+3] + (pData[IDX_DATA+2] >> 8);
            memcpy(&txStatusReport.rssi_values, &pData[IDX_DATA+5], MAX_REPEATERS + 1);
            txStatusReport.ackChannel = pData[IDX_DATA+10];
            txStatusReport.lastTxChannel = pData[IDX_DATA+11];
            txStatusReport.routeSchemeState = pData[IDX_DATA+12];
            memcpy(&txStatusReport.repeaters, &pData[IDX_DATA+13], MAX_REPEATERS);
            txStatusReport.routeSpeed = pData[IDX_DATA+17];
            txStatusReport.routeTries = pData[IDX_DATA+18];
            txStatusReport.lastFailedLinkFrom = pData[IDX_DATA+19];
            txStatusReport.lastFailedLinkTo = pData[IDX_DATA+20];
            f = cbFuncZWSendDataBridge;
            cbFuncZWSendDataBridge = 0;
            f( pData[ IDX_DATA + 1 ], &txStatusReport);//TODO add repeaters
          }
      }
      break;
   ...
   /*
    ...
    default:
      LIBSA_PRINTF("Unknown SerialAPI FUNC_ID: 0x%02x\n",pData[ IDX_CMD ]);
    }
  ...
  ) /*IN  Transmit completed call back function  */
{
  ...
  if ((dataLength + 2) > sizeof(buffer)) {
    LIBSA_PRINTF("ZW_SendData: Frame is too long\n");
    ASSERT(0);
    return FALSE;
  }
                                       ...
                                       VOID_CALLBACKFUNC(completedFunc)(BYTE txStatus))
{
  ...
  /* Unpack nodemask in byte array into array of node ids */
	int node_id = 0;
  for (node_id = 0; node_id < ZW_MAX_NODES; node_id++)
  {
