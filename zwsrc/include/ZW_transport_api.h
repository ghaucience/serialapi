 ...
 *
 * Please note: A few option flags below are mapped to the same bit. This is
 * not a problem since the interpretation of the flag bits depends on the
 * called Z-Wave API function. For example does no single API function
 * currently support both the LOW_POWER and MULTICAST_AS_BROADCAST options.
 * Hence the same flag bit (0x02) can safely be shared between those two
 * options.
 */
...
/**
 * Status report provided with callback from several Z-Wave API calls.
 */
typedef struct {
  ...
  /*
   * Single cast followup status
   * TODO: this might not be quite the right place for this info, but it makes implementation more easy
   */
  uint8_t followup_status[ZW_MAX_NODES];
} TX_STATUS_TYPE;
