  ...
  *c++=bChecksum;
  ...
  SerialPutBuffer(tx_buffer,len+5);
		/*
	{
			int x = 0;
			for (x = 0; x < len + 5; x++) {
				LIBCO_PRINTF("[%02X] ", tx_buffer[x]&0xff);
				if ( (x + 1) % 20 == 0) {
					LIBCO_PRINTF("\n");
				}
			}
    ...
    do
    {
      ...
      if(c<0) {
        break;
			...
			LIBCO_PRINTF("[RECV]: [%02X], con_state:%02X\n", c&0xff, con_state&0xff);
      switch (con_state)
      {
        case stateSOFHunt :
          if (c == SOF)
          ...
          SerialGetBuffer(serBuf+1,serBufLen);
					{
						LIBCO_PRINTF("[RECV]: ");	
						int xxx = 0;
						for (xxx = 0; xxx < serBufLen; xxx++) {
							LIBCO_PRINTF("[%02X] ", serBuf[xxx]&0xff);
							if ( (xxx +1) % 20 == 0) {
								LIBCO_PRINTF("\n[REEE]: ");
							}
						}
