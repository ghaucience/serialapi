...
#define PLATFORM_TIMER_SLOW_COMPENSATION_FACTOR	1 //if your platform is slow in processing the data, increase the timeouts by this factor
	...
	#define PORT_TIMER_RESTART(t) xtimer_restart(&t);
	#define PORT_TIMER_EXPIRED(t) xtimer_expired(&t)
	#ifdef __ASIX_C51__
		#define PORT_TIMER_INIT(t,msec) xtimer_set(&t, (msec * PLATFORM_TIMER_SLOW_COMPENSATION_FACTOR))
	#else
		#define PORT_TIMER_INIT(t,msec) xtimer_set(&t, msec);
	#endif
#else
	#ifdef _MSC_VER
		struct timer {
		...
		#define PORT_TIMER_EXPIRED(t) xtimer_expired(&t)
			...
			#define PORT_TIMER_INIT(t,msec) xtimer_set(&t, (msec * PLATFORM_TIMER_SLOW_COMPENSATION_FACTOR))
		#else
			#define PORT_TIMER_INIT(t,msec) xtimer_set(&t, (clock_time_t)msec)
		#endif
