...
const APPL_NODE_TYPE MyType = {0, 0};
	...
	switch(inf->bStatus) {
		...
		case ADD_NODE_STATUS_ADDING_CONTROLLER:
			if(inf->bLen) {
				...
				{
					printf("LEARN INFO: status:%02X\n", inf->bStatus&0xff);
					printf("LEARN INFO: source:%02X\n", inf->bSource&0xff);
					printf("LEARN CMDS: basic:%02x, generic:%02X, specific:%02X\n", inf->pCmd[0]&0xff, inf->pCmd[1]&0xff, inf->pCmd[2]&0xff);
					printf("LEARN CLAS:");
					int i = 0;
					for (i = 0; i < inf->bLen-3; i++) {
						printf("[%02X] ", inf->pCmd[i+3]&0xff);
					}
					printf("\n");
				}
			}
		...
		case ADD_NODE_STATUS_PROTOCOL_DONE:
			ZW_AddNodeToNetwork(ADD_NODE_STOP,AddNodeStatusUpdate);
			{
				...
				int i = 0;
				printf("LEARN CMDS: ");
				for (i = 0; i < inf->bLen; i++) {
					printf("[%02X] ", inf->pCmd[i]&0xff);
				}
				printf("\n");
			}
			break;
...
int main(int argc, char** argv) {
	...
	/** !!!!!!!!!!!!!!!
	 * use this can make the program to io multiplex, and you muse implemet a multiplex ctimer 
	 * int zfd  = SerialAPI_GetFd();
	 */
		...
		if(chready()) {
			switch(getc(stdin)) {
				...
				case 'v':
					...
					break;
				case '0': // SWITCH BINARY ON
					{
						unsigned char nid			= 0x01;		// you need to set to your test node id
						unsigned char class		= 0x25;		// the COMMAND_CLASS_SWITCH_BINARY
						unsigned char cmd			= 0x01;		// SWITCH_BINARY_SET
					  unsigned char data[1] = {0xFF}; // 0xFF->On, 0x00->Off
						zwave_class_cmd(nid, class, cmd, data, 1, NULL);
					}
					break;
				case '1': // SWITCH BINARY OFF
					{
						unsigned char nid			= 0x01;		// you need to set to your test node id
						unsigned char class		= 0x25;		// the COMMAND_CLASS_SWITCH_BINARY
						unsigned char cmd			= 0x01;		// SWITCH_BINARY_SET
						unsigned char data[1] = {0x00}; // 0xFF->On, 0x00->Off
						zwave_class_cmd(nid, class, cmd, data, 1, NULL);
					}
					break;
				case 'g':
					{
						TX_POWER_LEVEL pl = ZW_TXPowerLevelGet();
						printf("region: %02X, power:%02X, measu:%02X\n", ZW_RFRegionGet(), pl.normal&0xff, pl.measured0dBm&0xff);
					}
					break;
			}
		}
