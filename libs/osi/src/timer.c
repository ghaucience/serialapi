...
void timer_set(struct timer_head *head, struct timer *timer, _u64 ms)
{
	struct timer **prev;
	struct timer *node;
...
void timer_reset(struct timer_head *head, struct timer *timer,
		void (*handler)(struct timer *), _u64 ms)
{
	timer_cancel(head, timer);
...
_u64 timer_delay_get_ms(struct timer *timer)
{
	if (!timer->time_ms) {
		return 0;
	}
...
_s64 timer_advance(struct timer_head *head)
{
	struct timer *node;
	_u64 mtime = 0;
