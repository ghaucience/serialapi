...
struct timer {
	struct timer *next;
	_u64 time_ms;	/* monotonic trigger time */
...
void timer_set(struct timer_head *, struct timer *, _u64 delay_ms);
void timer_reset(struct timer_head *, struct timer *,
		void (*handler)(struct timer *), _u64 delay_ms);
_u64 timer_delay_get_ms(struct timer *);

/*
 * Handle timers and return delay until next timer fires.
 * Return -1 if no timers scheduled.
 */
_s64 timer_advance(struct timer_head *);
