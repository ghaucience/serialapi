...
static int lua_function(char *file, char *fname, int argc, char *argv[], char *out, int outsize) {
	...
	ret = lua_pcall(L, argc, 1, 0);
	if (ret != 0) {
		lua_close(L);
		return -4;
	}

	const char *sret = lua_tostring(L, -1);
	if (sret == NULL) {
		lua_close(L);
		return -5;
	}
...
int uproto_rpt_cmd_dusun(const char *extaddr, unsigned char ep, unsigned char clsid, unsigned char cmdid, const char *buf, int len) {
	...
	if (access("/etc/config/dusun/zwdev/zparse.lua", F_OK) == 0) {
		char out[1024];
		out[0] = 0;
		char epbuf[8];
		char clsbuf[8];
		char cmdbuf[8];
		char data[256];
		sprintf(epbuf,	"%02X", ep&0xff);
		sprintf(clsbuf, "%02X", clsid&0xff);	
		sprintf(cmdbuf, "%02X", cmdid&0xff);	
		int i = 0 ;
		for (i = 0; i < len; i++) {
			sprintf(data + i *2, "%02X", buf[i]&0xff);
		}
		char *argv[] = {
			epbuf, clsbuf, cmdbuf, data, NULL,
		};
		int argc = 4;
		int ret = lua_function("/etc/config/dusun/zwdev/zparse.lua", "parse_class_cmd", argc, argv, out, sizeof(out));
		//int ret = lua_function("/root/rbproto.lua", "callf", argc, argv, out, sizeof(out));
		log_warn("out:%s", out);
		json_error_t error;
		json_t *jret = json_loads(out, 0, &error);
		if (jret != NULL) {
			char *sattr = json_get_string(jret, "attribute");
			char attr_buf[128];
			sprintf(attr_buf, sattr);
			if (sattr != NULL) {
				json_object_del(jret, "attribute");
				_uproto_handler_cmd("", "", "", "", 0, "", "reportAttribute", device_make_macstr(zd), attr_buf, jret);
			} else {
				json_decref(jret);
				log_warn("lua parse result no attribute");
			}
		} else {
			log_warn("lua parse result is not a json");
		}
		return 0;
	}
...
void __ApplicationControllerUpdate(BYTE bStatus, /*IN  Status event */
...
void __ApplicationNodeInformation(BYTE *deviceOptionsMask, /*OUT Bitmask with application options    */
...
static void __ApplicationCommandHandler(
