...
static int gpio_set_dir(int r, int dir) {
    int fd, req;
...
static int gpio_read_int(int r, int *value) {
    int fd, req;
...
static int gpio_write_int(int r, int value) {
    int fd, req;
...
static int gpio_reg_info(int gpio_num) {
    int fd;
...
static void gpio_test_intr(int gpio_num) {
...
static void gpio_set_led(int num, int on, int off, int blinks, int rests, int times) {
	int fd;
	ralink_gpio_led_info led;
...
typedef struct stLedGpioMap {
	char *name;
	int   gpio;
} stLedGpioMap_t;

static stLedGpioMap_t lgms[] = {
	{"zigbee", 63},
	{"pwrled", 62},
	{"errled", 61},
};

static stLedGpioMap_t lgms_24g5g[] = {
	{"zigbee", 42},
	{"pwrled", 62},
	{"errled", 61},
};

static stLedGpioMap_t lgms_0177[] = {
	{"zigbee", 19},
	{"pwrled", 4},
	{"errled", 5},
};
...
static stLedGpioMap_t *led_search(char *name) {
	int i = 0;

	int cnt = sizeof(lgms)/sizeof(lgms[0]);
	if (access("/tmp/sysinfo/item", F_OK) == 0) {
		cnt = sizeof(lgms_24g5g)/sizeof(lgms_24g5g[0]);
	} else if (strcmp(system_model_get(), "DSI0177") == 0) {
		cnt = sizeof(lgms_0177)/sizeof(lgms_0177[0]);
	}

	for (i = 0; i < cnt; i++) {
		stLedGpioMap_t *lgm = &lgms[i];
		if (access("/tmp/sysinfo/item", F_OK) == 0) {
			lgm = &lgms_24g5g[i];
		} else if (strcmp(system_model_get(), "DSI0177") == 0) {
			lgm = &lgms_0177[i];
		}
		if (strcmp(lgm->name, name) != 0) {
			continue;
		}
		return lgm;
	}
...
int mtsdk_led_get(char *led) {
	stLedGpioMap_t *lgm = led_search(led);
	if (lgm == NULL) {
		return 0;
	}
	return 0;
...
int mtsdk_led_on(char *led) {
	...
	gpio_set_led(lgm->gpio, 4000, 0, 1, 0, 4000);
	return 0;
}
...
int mtsdk_led_off(char *led) {
	...
	gpio_set_led(lgm->gpio, 0, 4000, 0, 1, 4000);
	return 0;
}
...
int mtsdk_led_blink(char *led, int delay_on, int delay_off) {
	...
	int on	= (delay_on+99)/100;
	int off	= (delay_off+99)/100;
	
	gpio_set_led(lgm->gpio, on, off, 4000, 0, 4000);
	return 0;
}
...
int mtsdk_led_shot(char *led){
	...
	gpio_set_led(lgm->gpio, 1, 1, 0, 0, 0);
	return 0;
}
