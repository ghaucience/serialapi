...
void ZCB_ts_senddata_cb(unsigned char status_send, TX_STATUS_TYPE* ts);
#endif
void fire_rx_timer();
void fire_fc_timer();
...
uint8_t TS_SEND_RAW(uint8_t snode, uint8_t dnode, uint8_t *cmd, uint8_t len, uint8_t flags, VOID_CALLBACKFUNC(completedFunc)(uint8_t, TX_STATUS_TYPE*))
{
