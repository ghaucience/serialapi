...
{
  return S2_send_data_both_cast(p_context, dst, buf, len, SEND_MSG);
}
...
S2_send_data_multicast(struct S2* p_context, const s2_connection_t* con, const uint8_t* buf, uint16_t len)
 {
   return S2_send_data_both_cast(p_context, con, buf, len, SEND_MULTICAST);
 }
...
S2_send_data_both_cast(struct S2* p_context, const s2_connection_t* con, const uint8_t* buf, uint16_t len, event_t ev)
{
  ...
  if (ev == SEND_MULTICAST) {
    dbg_printf("S2 send data multi %i->[%i] class %i\n",con->l_node,con->r_node,con->class_id);
