...
extern WORD zgw_crc16(uint16_t crc16, uint8_t *data, unsigned long data_len);

#else // ifdef ZIPGW

extern WORD zgw_crc16(WORD crc, BYTE *pDataAddr, WORD bDataLen);
...
#ifndef ZIPGW
typedef void TX_STATUS_TYPE; /* For NEW_TEST_T2 */
#endif
...
typedef uint16_t clock_time_t; /* For NEW_TEST_T2 */

struct ctimer { /* For NEW_TEST_T2 */
    struct ctimer *next;
...
extern uint8_t TS_SEND_RAW(uint8_t snode, uint8_t dnode, uint8_t *cmd, uint8_t len, uint8_t flags, VOID_CALLBACKFUNC(completedFunc)(uint8_t, TX_STATUS_TYPE*));

extern void ctimer_set(struct ctimer *c, clock_time_t t, /* For NEW_TEST_T2 */
                       void (*f)(void *), void *ptr);
extern void ctimer_stop(struct ctimer *c); /* For NEW_TEST_T2 */
...
{
    uint8_t *tmp_buf = buf;
    WORD crc = zgw_crc16(0x1D0F, tmp_buf, len);
    tmp_buf+=len;
    *tmp_buf++ =  (crc>>8)&0xff;
    *tmp_buf=(crc)&0xff;
}
...
#define SUBSEQ_FRAG_NONPAYLOAD_LENGTH (sizeof(ZW_COMMAND_SUBSEQUENT_FRAGMENT_1BYTE_FRAME) - 1)
        ...
        if (zgw_crc16(0x1D0F, (uint8_t*) rcb.fragment, rcb.fragment_len) != 0) {
