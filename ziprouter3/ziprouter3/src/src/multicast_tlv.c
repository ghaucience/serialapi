  ...
  while ( (BYTE*)tlv < tail_payload)
  {
    switch(tlv->type & ZIP_EXT_HDR_OPTION_TYPE_MASK) {
      ...
      case MULTICAST_DESTINATION_ADDRESS:
        ...
        txOpts = txOpts | TRANSMIT_OPTION_MULTICAST;
        ...
        while((BYTE*)mcast_tlv < mcast_payload) {
          ...
          if (is_4to6_addr((ip6addr_t *)&zw_conn->ripaddr)) {
            uip_ipv4addr_t ip4;
...
#ifdef TEST_MULTICAST_TX
uint16_t gen_mcast_ack_value(const zwave_connection_t *zw_conn, const mcast_status_t mcast_status[], u8_t num_nodes, u8_t *buf, u16_t buf_size)
{
  int i = 0;
  u16_t count = 0;
  uip_ipv4addr_t ip4;
