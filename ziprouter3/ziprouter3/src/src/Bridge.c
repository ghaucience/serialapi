...
void
reset_fw_timeout(void *user)
{
  struct IP_association* a = user;
  DBG_PRINTF("Resetting fw flag for IP association %x\n", (unsigned int)a);
  IP_association_fw_lock.locked_a = 0;
}
...
struct IP_association unsol_ip_assoc;
void
bridge_init()
{
  uint8_t cap;
...
get_oldest_temporary_association()
{
  ...
  if (update_temp_assoc_count() == 0) {
      LOG_PRINTF("initializing IP_association_fw\n");
      IP_association_fw_lock.locked_a = 0;
      ctimer_stop(&IP_association_fw_lock.reset_fw_timer);
  }
  /* create a new assoc, if we have fewer than expected */
  DBG_PRINTF("we have %d temp associations\n", update_temp_assoc_count());
  if (update_temp_assoc_count() < PREALLOCATED_VIRTUAL_NODE_COUNT)
  {
    DBG_PRINTF("Creating new temp assoc as we have only %d temp associations which is less than %d\n", update_temp_assoc_count(), PREALLOCATED_VIRTUAL_NODE_COUNT);
    a = (struct IP_association*) memb_alloc(&association_pool);
    a->type = temporary_assoc;
    DBG_PRINTF("New association is: %x \n", (unsigned int)a);
    return a;
  }

  for (a = list_head(association_table); a != NULL; a = list_item_next(a))
  {
    DBG_PRINTF("Checking %x IP_association_fw_lock.locked_a: %x\n", (unsigned int)a, (unsigned int)IP_association_fw_lock.locked_a);
    DBG_PRINTF("ip association %d->%d\n", a->virtual_id, a->han_nodeid);
    if ((a->type == temporary_assoc) && (IP_association_fw_lock.locked_a != a))
    {
      list_remove(association_table, a);
      DBG_PRINTF(":virtual_id: %d:\n", a->virtual_id);
      DBG_PRINTF("Reusing:%x \n", (unsigned int)a);
      return a;
    }
  }
...
find_existing_assoc(enum ASSOC_TYPE type,uint8_t was_dtls)
{
  ...
  DBG_PRINTF("find_existing_assoc\n");
  destination_node = nodeOfIP(&(BACKUP_UIP_IP_BUF->destipaddr));

  /* We can have a permanant association between zwave node and Unsolicitied destiantion. And we dont want that to be handled here */
  if((type == temporary_assoc) &&
     (uip_ipaddr_cmp(&cfg.unsolicited_dest,&(BACKUP_UIP_IP_BUF->srcipaddr))) && (UIP_HTONS(BACKUP_UIP_UDP_BUF->srcport) == cfg.unsolicited_port)) {
    a = &unsol_ip_assoc;
    a->resource_endpoint = BACKUP_ZIP_PKT_BUF->sEndpoint;
    a->type = temporary_assoc;
    a->was_dtls = was_dtls;
    a->han_nodeid = destination_node;
    unsol_ip_assoc.resource_port = cfg.unsolicited_port;
    uip_ipaddr_copy(&(unsol_ip_assoc.resource_ip), &(cfg.unsolicited_dest));
    WRN_PRINTF("find_existing_assoc: Packet is from Unsol destination Allocating static unsol IP association: %x\n", (unsigned int) a);
    WRN_PRINTF("Source node will be GW's Node id \n");
  ...
  {
#if 0
    DBG_PRINTF("Matching Ip assoc type: %d, han_nodeid: %d, resource_port: %d, resource_endpoint: %d, was_dtls: %d\n"
               "type: %d,  dnode: %d, buf->srcport: %d, buf->sendpoint: %d, was_dtls:%d\n",
               a->type, a->han_nodeid, a->resource_port, a->resource_endpoint, a->was_dtls,
               type, destination_node, BACKUP_UIP_UDP_BUF->srcport, BACKUP_ZIP_PKT_BUF->sEndpoint, was_dtls);
#endif
    ...
    if(
      ...
      DBG_PRINTF("find_existing_assoc found: %x\n", (unsigned int)a);
      return a;
    }
  }
  DBG_PRINTF("IP association NOT found\n");
  return NULL;
}
...
(*cb_func)(struct IP_association *), ZW_COMMAND_IP_ASSOCIATION_SET *ip_assoc_set_cmd, uint8_t was_dtls, uint8_t fw)
{
  ...
  if (type == temporary_assoc)
  {
    ...
    {
      DBG_PRINTF("failed to alloc or recycle a struct IP_assoc\n");
      /* Can happen if a non-responding SIS causes virtual node creation to fail */
      do_bridge_callback(NULL);
      return BRIDGE_FAIL;
    }
    DBG_PRINTF("Re-using oldest temporary association with virtual nodeid %d %x %d\n", new_assoc->virtual_id, (unsigned int)new_assoc, fw);
  }
...
SET_FW_FLAG:
  /*We set firmware flag here because the virtual nodes are allocated here and not in get_oldest_temporary_association */
  /*The code flow also comes here from find_existing_assoc and the firmware flag has to set again to reset the timer */ 
  if ((type == temporary_assoc) && fw) {
     DBG_PRINTF("Setting fw flag for IP association %x %d->%d\n", (unsigned int)new_assoc, new_assoc->virtual_id, new_assoc->han_nodeid);
     if (IP_association_fw_lock.locked_a) {
        DBG_PRINTF("Previous fw locked ip association was%x %d->%d\n", (unsigned int)IP_association_fw_lock.locked_a, IP_association_fw_lock.locked_a->virtual_id, IP_association_fw_lock.locked_a->han_nodeid);
     }
     IP_association_fw_lock.locked_a = new_assoc; 
     ctimer_set(&IP_association_fw_lock.reset_fw_timer, 60000, reset_fw_timeout, new_assoc);
  }
  return BRIDGE_OK;
}
...
remove_bridge_association(struct IP_association* a)
{
  ...
  assert(a->type != temporary_assoc);

  if ( !a || (a == &unsol_ip_assoc)) {
     WRN_PRINTF("Tried to remove static IP association made for unsol destination traffic");
     return;
  }  

  if ( a == IP_association_fw_lock.locked_a) {
      IP_association_fw_lock.locked_a = 0;
      ctimer_stop(&IP_association_fw_lock.reset_fw_timer);
  }
