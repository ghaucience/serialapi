...
typedef struct ep_probe_done_notifier
{
    ...
    * ZIP_NAMING_LOCATION_GET, \ref NodeProbeDone() for \a NODE_ADD,
    * or \ref NodeInfoCachedReport() for \a NODE_INFO_CACHED_GET. */
  void
...
rd_check_for_dead_nodes_worker(void* v)
{
  ...
  for (i = start_with_node; i < ZW_MAX_NODES - 1; i++)
  {
    ...
    if (((n->wakeUp_interval == 0) || (n->mode & MODE_FLAGS_FAILED)) && ((n->mode & 0xff) == MODE_MAILBOX))
    {
...
/*
 * Remove the global portable flag when we handle the eeprom convert
 */
...
void
rd_register_new_node(uint8_t node, uint8_t node_properties_flags)
{
  rd_node_database_entry_t *n;
  rd_ep_database_entry_t * ep;
  //char name[16];
...
/**
 * Validate a the Name part of the service name based on spec SDS13782 Z/IP Naming and Location CC
 ...
 * The location may contain . but not start or end with them
 ...
 Each location sub-string (separtaed by the dot) MUST NOT end with dash (SDS13782)
 */
u8_t
validate_location(char* LocationName, u8_t len)
{
  int i;
  
  if (LocationName[0] == '.' || LocationName[len - 1] == '.')
    return 0;
  for (i = 0; i < len; i++)
  {
    if (LocationName[i] == '_')
  ...
  for (i = 1; i < (len - 1); i++)
  {
    if (LocationName[i] == '.' && LocationName[i - 1] == '-')
      return 0;
  }
  return 1;
}
...
int rd_check_security_for_unsolicited_dest( uint8_t rnode,  security_scheme_t scheme, void *pData, u16_t bDatalen )
{
  ...
  if(CommandAnalyzerIsSupporting(pCmd->ZW_Common.cmdClass, pCmd->ZW_Common.cmd)) {
   ...
   /* Some non-secure legacy sensor devices send BASIC SET. We need to let these kind of BASIC SETs to unsol destinatino.
    * But we also need to make sure that supervision encapsulated BASIC Set are dropped and replied with NO_SUPPORT */
    /* So we allow BASIC cmd class if its send from non-secure node which does nt support Supervision */
    ...
    //Is this set in the secure part of the NIF, or BASIC which must always be supported
    if(  (pCmd->ZW_Common.cmdClass == COMMAND_CLASS_BASIC ) || IsCCInNodeInfoSetList(pCmd->ZW_Common.cmdClass,TRUE)) {
      if(scheme == net_scheme) {
        return TRUE;
      }
    }
    //Is it in the non-secure part, or SUPERVISION which shoudl also be handled by the backend.
    if( (pCmd->ZW_Common.cmdClass == COMMAND_CLASS_SUPERVISION) ||
        IsCCInNodeInfoSetList(pCmd->ZW_Common.cmdClass,FALSE)) {
      return TRUE;
    }
  }
