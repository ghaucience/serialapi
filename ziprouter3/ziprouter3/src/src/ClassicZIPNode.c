...
extern nodeid_t temp_assoc_virtual_nodeids[PREALLOCATED_VIRTUAL_NODE_COUNT];
void reset_fw_timeout(void *user);
 ...
 */
static void
unsolicited_senddata_callback_worker(BYTE bStatus, void* user, TX_STATUS_TYPE *t, BOOL is_mcast)
{
  ctimer_stop(&nak_wait_timer);
      ...
      && (bStatus != TRANSMIT_COMPLETE_OK && bStatus != TRANSMIT_COMPLETE_ERROR)))
  {
    ...
    {
      SendUDPStatus_rssi( bStatus == TRANSMIT_COMPLETE_OK ? ZIP_PACKET_FLAGS0_ACK_RES :ZIP_PACKET_FLAGS0_NACK_RES , cur_flags1,&zw, t, is_mcast);
    }
  }
...
#ifdef TEST_MULTICAST_TX
/* Callback from senddata on unsolicited multicast frame.
 * Send a ZIP Ack/Nack and reset ip association state.
 *
 * Preconditions:
 *   - current_ip_association must be setup
 */
static void
unsolicited_senddata_callback_mcast(BYTE bStatus, void* user, TX_STATUS_TYPE *t)
{
  unsolicited_senddata_callback_worker(bStatus, user, t, TRUE);
}
#endif
...
{
  unsolicited_senddata_callback_worker(bStatus, user, t, FALSE);
}
...
handle_ip_association_set(zwave_connection_t* c, const u8_t* payload, const u8_t len,BOOL was_dtls)
{
  ...
  if (is_local_address(assoc_dest_ip))
  {
       ...
       )
    {
      ...
      if (create_bridge_association(proxy_assoc,
          handle_ip_assoc_case2_callback, ip_assoc_set_cmd,was_dtls, 0) != BRIDGE_OK)
      {
        send_zipack_ipassoc(TRANSMIT_COMPLETE_ERROR);
        ASSERT(0);
        reset_ip_association_state(TRANSMIT_COMPLETE_ERROR);
  ...
  else /* if (is_local_address(assoc_dest_ip)) */
  {
    if (create_bridge_association(permanent_assoc,
        handle_ip_assoc_external_callback, ip_assoc_set_cmd,was_dtls, 0) != BRIDGE_OK)
    {
      send_zipack_ipassoc(TRANSMIT_COMPLETE_ERROR);
      ASSERT(0);
      reset_ip_association_state(TRANSMIT_COMPLETE_ERROR);
...
temp_assoc_callback(struct IP_association *a)
{
  ...
  if (current_ip_association)
  {
    ...
    if (txOpts & TRANSMIT_OPTION_MULTICAST) {
      DBG_PRINTF("Mcast callback requested");
        cur_handle = ZW_SendDataAppl(&p, (BYTE*) &classic_txBuf, zip_payload_len,
            unsolicited_senddata_callback_mcast, 0);
    } else {
#endif
        cur_handle = ZW_SendDataAppl(&p, (BYTE*) &classic_txBuf, zip_payload_len,
...
proxy_command_handler(zwave_connection_t* c, const u8_t* payload, u8_t len, BOOL was_dtls, BOOL ack_req, uint8_t bSupervisionUnwrapped)
{
  ...
  case COMMAND_CLASS_ZIP_NAMING:
    ...
    return ZW_command_handler_run(&tc, (uint8_t*) payload, len, bSupervisionUnwrapped) == COMMAND_HANDLED;
 ...
 *    if (create_bridge_association() != OK):
...
int ClassicZIPUDP_input(struct uip_udp_conn* c, u8_t* pktdata, u16_t len, BOOL secure)
{
  ...
  DBG_PRINTF("ClassicZIPUDP_input len: %d\n", len);
...
//  ERR_PRINTF("Setting seq[%d]:  to %x\n", nodeOfIP(&c->sipaddr), zip_ptk->seqNo);
    ...
    {
      reset_ip_association_state(TRANSMIT_COMPLETE_REQUEUE_QUEUED);
    ...
    if (create_bridge_association(temporary_assoc, temp_assoc_callback, NULL,secure,
          ((payload[0] == COMMAND_CLASS_FIRMWARE_UPDATE_MD) && //If its firmware update md get or report
...
*/
int
ClassicZIPNode_input(int node, VOID_CALLBACKFUNC(completedFunc) (BYTE,BYTE*,uint16_t),
...
LogicalRewriteAndSend()
{
  ...
  if (UIP_IP_BUF ->destipaddr.u16[5] == MANGLE_MAGIC)
  {
    ...
    if (a)
    {
      ...
      {
        DBG_PRINTF("LogicalRewriteAndSend: setting %x fw flag", (unsigned int)a);   
        /* We could end up reusing temporary association already created for OTA upgrade and this causes
         * OTA to stall so we need to mark it with
        ...
        IP_association_fw_lock.locked_a = a;
        DBG_PRINTF("new one fw locked ip association was%x %d->%d\n", (unsigned int)IP_association_fw_lock.locked_a, IP_association_fw_lock.locked_a->virtual_id, IP_association_fw_lock.locked_a->han_nodeid);
        ctimer_set(&IP_association_fw_lock.reset_fw_timer, 60000, reset_fw_timeout, a);
      }
      ...
      DBG_PRINTF("Packet from Z-wave side (nodeid: %d) to port:%d IP addr: ", snode, UIP_HTONS(c.rport));
      uip_debug_ipaddr_print(&c.ripaddr);
