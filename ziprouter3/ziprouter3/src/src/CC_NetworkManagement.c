...
/** Send a NODE_LIST_REPORT to nms.conn.
 *
 * This function can be called in all NM states, so it should not
 * affect GW state in any way.  It sends the nodelist as perceived by
 * the protocol.
 *
 * The only exception is in state #NM_NETWORK_UPDATE, when GW is
 * processing a #NETWORK_UPDATE_REQUEST asynchronously because GW is
 * not SUC/SIS.  Then any new nodes will be probed and
 * SendReplyWhenNetworkIsUpdated() is called to set up TX of the
 * nodelist when the probe completes.
 */
static void SendNodeList(BYTE bStatus);
...
static void pvl_set_name_and_location(void *usr)
{
  ...
  {
    rd_update_ep_name_and_location(ep, (char*)name_tlv->value, name_tlv->length,
                                (char*)loc_tlv->value, loc_tlv->length);
  }
      ...
      if(nms.cmd == NODE_ADD) {
        if (ep->state == EP_STATE_PROBE_DONE)
        {
          ...
          /* Save space for DSK, dsk length, supported keys and reserved fields in node add status command */
             max = (sizeof(nms.raw_buf) - offsetof(ZW_NODE_ADD_STATUS_1BYTE_FRAME,commandClass1)) -  (3 + sizeof(nms.just_included_dsk));
          } else {
        ...
        ZW_FAILED_NODE_REPLACE_STATUS_FRAME_EX* reply = (ZW_FAILED_NODE_REPLACE_STATUS_FRAME_EX*) &nms.buf;
        reply->grantedKeys = sec2_gw_node_flags2keystore_flags(GetCacheEntryFlag(nms.tmp_node));
...
skip:
    ...
    break;
  case NM_WAIT_DHCP:
    if ((ev == NM_EV_DHCP_DONE && nms.tmp_node == *(uint8_t*) event_data) || (ev == NM_EV_TIMEOUT))
    {
      if((cfg.mb_conf_mode != DISABLE_MAILBOX) && rd_get_node_mode(nms.tmp_node) == MODE_MAILBOX ) {
        mb_put_node_to_sleep_later(nms.tmp_node);
      }
...
LearnModeStatus(LEARN_INFO* inf)
{
  ...
  case LEARN_MODE_DONE:
    ...
    if (clean_network || inf->bSource == 0)
    {
      ...
      ipv6nat_rename_node(old_nodeid, MyNodeID);
  ...
  case LEARN_MODE_FAILED:
       ...
       * timeout handler has already called ApplicationInitSW() and
...
NodeInfoCachedReport(rd_ep_database_entry_t* ep, void* user)
{
  ...
  if (ep)
  {
    ...
    {
      size_t max = sizeof(nms.raw_buf) - offsetof(ZW_NODE_INFO_CACHED_REPORT_1BYTE_FRAME, nonSecureCommandClass1);
...
//  nms.buf.ZW_Common.cmdClass = pCmd->ZW_Common.cmdClass;
  ...
  case COMMAND_CLASS_NETWORK_MANAGEMENT_INCLUSION:
    ...
    case FAILED_NODE_REMOVE:
      ...
      BYTE ret = 0;
      ret = ZW_RemoveFailedNode(pCmd->ZW_FailedNodeRemoveFrame.nodeId,
...
BOOL NetworkManagement_SendNodeList_To_Unsolicited()
{
  ...
  if (uip_is_addr_unspecified(&cfg.unsolicited_dest)
      ...
      DBG_PRINTF("No Unsolicited Destination, not sending node list\n");
  }
