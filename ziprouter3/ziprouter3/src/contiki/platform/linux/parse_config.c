...
#include "ZIP_Router.h" // for struct cfg
...
static void parse_prog_args()
{
  ...
  while ((opt = getopt(prog_argc, prog_argv, "mnc:s:f:e:t:p:w:")) != -1)
  {
    switch (opt)
    {
    case 'n':
    ...
    case 'x':
      ...
      break;
    default: /* '?' */
      fprintf(stderr, "Usage: %s [-c cfgfile] [-s serial_dev] [-f serial_log] [-e eeprom_file] [-p provisioning_config_file] [-w provisioning_list_storage_file] [-t tun_script]\n",
          prog_argv[0]);
      exit(EXIT_FAILURE);
    }
  }
